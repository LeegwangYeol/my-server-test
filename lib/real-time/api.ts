...
export class RealtimeAPI extends RealtimeEventHandler {
   ...
   */
  async connect({ model }: { model?: string } = { model: 'gpt-4o-realtime-preview-2024-10-01' }): Promise<true> {
   ...
   * @returns {true}
   */
  receive(eventName: string, event: { [key: string]: any }): true {
   ...
   * @returns {true}
   */
  send(eventName: string, data?: { [key: string]: any }): true {
    if (!this.isConnected()) {
