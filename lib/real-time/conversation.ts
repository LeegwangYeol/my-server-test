...
export class RealtimeConversation {
  ...
  EventProcessors: { [key: string]: Function } = {};
  itemLookup: { [id: string]: ItemType } = {};
  items: ItemType[] = [];
  responseLookup: { [id: string]: any } = {};
  responses: any[] = [];
  queuedSpeechItems: { [id: string]: any } = {};
  queuedTranscriptItems: { [id: string]: any } = {};
  ...
  constructor() {
    ...
    this.EventProcessors = {
      'conversation.item.created': (event: any) => {
        const { item } = event;
        // deep copy values
        const newItem: ItemType = JSON.parse(JSON.stringify(item));
        ...
        if (newItem.content) {
          const textContent = newItem.content.filter((c: any) => ['text', 'input_text'].includes(c.type));
   ...
   * @param {Object} event
   * @param  {...any} args
   * @returns {item: ItemType | null, delta: ItemContentDeltaType | null}
   */
